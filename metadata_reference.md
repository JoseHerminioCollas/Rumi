...
The `attributes` array uses `trait_type` and `value` pairs to categorize the specimen:
...
*   **`REINFO Status`**: **Values like** `Vigente` or `Formalizado`, proving the miner's standing at the moment of minting.
*   **`Legal Status`**: A summary of the stone's compliance status (e.g., `Certified Origin & Possession`).
